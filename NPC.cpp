...
void Non_Character::options(Character* player)
{
	...
	while (quit == 0)
	{
		if (npc_name != "Wieœniak")
		{
			...
			if (npc_name == "Kowal Gregori" || npc_name == "Kowal Andrzej")		std::cout << "'3' - naprawa zbroi" << std::endl;
...
void Non_Character::buy(Character* player)
{
	...
	if (npc_type == 'C') //chemicy
	{
		...
		{
			potions[i].eq_cost = 100 * (potions[i].eq_stats[0] + potions[i].eq_stats[1] + potions[i].eq_stats[2] + potions[i].eq_stats[3] + potions[i].eq_stats[4] + potions[i].eq_stats[5] + seller);
		}
...
void Non_Character::talk(Character* player)
{
	if (player->GetStats().getByEnum(Charisma) < 25)
	{
		if (npc_name == "Chemik Renagan")
		{
			std::cout << "No wiesz, kiedyœ to tu by³a ³adne miasteczko, du¿o zieleni tu by³o," << std::endl;
	...
	else if(player->GetStats().getByEnum(Charisma) >= 25)
	{
		...
		else if (npc_name == "Handlarz")
		{
			...
			Sleep(1000);
			std::cout << "Wiec... \"to zaczê³o sto piêædziesi¹t lat temu, za mojego pra pra dziadka." << std::endl;
			...
			Sleep(1000);
			std::cout << "A prenego poranka, to ja ci powiem szczerze..." << std::endl;
