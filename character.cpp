...
void Character::EquipWeapon() {
	...
	int choice;
	std::cin >> choice;
	if (choice - 1 < 0 || choice - 1 > weapon_eq.size()) {
		std::cout << "Niepoprawny wybór!" << std::endl;
		weapon_dmg = previous_dmg;
	}
	else {
		weapon_dmg = weapon_eq[choice - 1].dmg_counter(this);
	}
}
...
void Character::EquipArmor() {
	...
	std::cin >> choice;
	choice--;
	if (choice < 0 || choice > armor_eq.size() || armor_eq[choice].armor_name != "Napiersnik") {
		std::cout << "Niepoprawny wybór!" << std::endl;
